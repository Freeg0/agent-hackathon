...
async function main() {
  ...
  console.log(
    `   ${
      useLLM
        ? '⚡ Using mock LLM for demographic analysis'
  ...
  if (result.analysisResult) {
    ...
    console.log('\nAge Distribution:')
    for (const [bucket, percent] of Object.entries(
      analysis.populationCoverage.age
    )) {
      console.log(`  ${bucket}: ${percent}%`)
    }

    console.log('\nGender Distribution:')
    for (const [gender, percent] of Object.entries(
      analysis.populationCoverage.gender
    )) {
    ...
    if (analysis.populationCoverage.pregnancyStatus) {
      console.log('\nPregnancy Status:')
      for (const [status, percent] of Object.entries(
        analysis.populationCoverage.pregnancyStatus
      )) {
    ...
    if (analysis.populationCoverage.geography) {
      ...
      for (const [region, percent] of Object.entries(
        analysis.populationCoverage.geography
      )) {
  ...
  if (result.reportSummary) {
    ...
    console.log('\n📊 Key Metrics:')
    console.log(
      `   Total Papers: ${result.reportSummary.keyMetrics.totalPapers}`
    )
    console.log(
      `   High Quality Papers: ${result.reportSummary.keyMetrics.highQualityPapers}`
    )
    ...
    if (result.reportSummary.mainBlindSpots.length > 0) {
      ...
      result.reportSummary.mainBlindSpots.forEach((bs, idx) => {
        console.log(
          `   ${idx + 1}. [${bs.severity.toUpperCase()}] ${bs.category}: ${
            bs.gap
          }`
        )
      })
  ...
  if (result.papers.length > 0) {
    ...
    if (years.length > 0) {
      ...
      console.log(
        `   Papers after quality filter: ${result.highQualityPapers.length}`
      )
